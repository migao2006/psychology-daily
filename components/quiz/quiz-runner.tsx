...
export function QuizRunner({ lesson, nextLessonSlug }: { lesson: Lesson; nextLessonSlug?: string }) {
  ...
  async function finish() {
    setSaving(true);
    await completeLesson({ lessonId: lesson.id, familiarity, concepts: lesson.quiz.map((question) => ({ questionId: question.id, conceptId: question.conceptId })), answers: lesson.quiz.map((question) => ({ questionId: question.id, selectedIndex: selected[question.id], correct: selected[question.id] === question.correctIndex, answeredAt: new Date().toISOString() })) });
    router.push("/"); router.refresh();
  }
  return <section className="card" aria-labelledby="quiz-title"><h2 id="quiz-title">用 {lesson.quiz.length} 題確認理解</h2><p className="muted">答案送出後才會顯示解析；這不是心理疾病或人格測驗。</p>
