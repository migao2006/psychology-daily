...
- 每日自動回補最多 50 篇，主要搜尋最近 180 天，連續無進展時依規則擴大至 365 天。
- 支援 Groq 嚴格 JSON Schema 作為免費高吞吐量 provider，並保留既有 Gemini／OpenAI provider。
...
- 回補目標為研究庫總數 100，單批最多新增 50 篇且永不因回補超過剩餘篇數。
- Groq、Gemini 與 OpenAI 使用相同摘要 schema、第二次 grounding 查核與失敗保護。
