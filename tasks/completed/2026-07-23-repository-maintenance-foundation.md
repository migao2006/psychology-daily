...
- 建立長期 Repository guide、文件責任索引、work package lifecycle 與精簡 continuity。
- 集中並測試研究摘要 Prompt，未改變 provider、schema 或產品行為。
- 移除無引用樣板資產與一行式轉接檔，補齊 generated files 與 link-check 範圍。
- 全部本機驗證通過。
- 功能分支已 push；Draft PR：`https://github.com/migao2006/psychology-daily/pull/2`。
- 未更新或合併 `main`，未執行 Production 或 Worker deployment。
