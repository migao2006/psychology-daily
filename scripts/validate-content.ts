...
import backfillStateJson from "@/content/research/backfill-state.json";
import { researchBackfillStateSchema } from "@/lib/schemas/research";
...
const backfillState = researchBackfillStateSchema.parse(backfillStateJson);
if (backfillState.status === "completed" && researchIndex.items.length < backfillState.target) {
  throw new Error("Completed backfill state requires the research target");
}
