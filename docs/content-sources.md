...
獨立 `backfill-research.yml` 每日聚合 OpenAlex、Europe PMC 與 Crossref；只有某組查詢在三個主要來源都沒有合格候選時，才使用 Semantic Scholar fallback。每批最多 50 篇，目標 100 篇、六類各至少 12 篇、預印本最多 20 篇。它先查最近 180 天；連續三批沒有新增才擴至 365 天，再連續三批無新增就停止。分類不足時每批優先查詢篇數最少的兩類，最終仍以確定性品質分數排序。

永久候選問題（metadata 不一致、無效來源、摘要 schema／grounding／第二次查核失敗）會以 DOI 或正規化標題記入 `backfill-state.json`，避免每天重複付費。429、5xx、網路錯誤或設定問題會中止整批，且在任何研究檔案寫入前失敗。只有完整批次通過所有驗證後，workflow 才可提交 `content/research/`；每日更新與回補共用 concurrency group，避免覆蓋彼此。
