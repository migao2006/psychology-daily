...
獨立 `backfill-research.yml` 每日聚合 OpenAlex、Europe PMC 與 Crossref；只有某組查詢在三個主要來源都沒有合格候選時，才使用 Semantic Scholar fallback。每批最多 10 篇，目標 100 篇、六類各至少 12 篇、預印本最多 20 篇。它先查最近 180 天；連續三批沒有新增才擴至 365 天，再連續三批無新增就停止。分類不足時每批優先查詢篇數最少的兩類，最終仍以確定性品質分數排序。
